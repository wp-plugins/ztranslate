...
/* zTranslate Core Functions */

function ztrans_init() {
	global $q_config;
	// check if it isn't already initialized
	if(defined('ZTRANS_INIT')) return;
	define('ZTRANS_INIT',true);
	// load configuration if not beeing reseted
	if(isset($_POST['ztranslate_reset']) && isset($_POST['ztranslate_reset2']) && defined('WP_ADMIN') && current_user_can('manage_options')) {
		// reset all settings
	...
	ztrans_loadConfig();
	if(isset($_COOKIE['ztrans_cookie_test'])) {
...
//	ztrans_initJS();
	...
	if(defined('WP_ADMIN') && $q_config['auto_update_mo']) ztrans_updateGettextDatabases();
	
	// update definitions if neccesary
	if(defined('WP_ADMIN') && current_user_can('manage_categories')) ztrans_updateTermLibrary();
	
	// extract url information
	$q_config['url_info'] = ztrans_extractURL($_SERVER['REQUEST_URI'], $_SERVER["HTTP_HOST"], isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '');
	
	// set test cookie
	setcookie('ztrans_cookie_test', 'zTranslate Cookie Test', 0, $q_config['url_info']['home'], $q_config['url_info']['host']);
	// check cookies for admin
	if(defined('WP_ADMIN')) {
		if(isset($_GET['lang']) && ztrans_isEnabled($_GET['lang'])) {
			$q_config['language'] = $q_config['url_info']['language'];
			setcookie('ztrans_admin_language', $q_config['language'], time()+60*60*24*30);
		} elseif(isset($_COOKIE['ztrans_admin_language']) && ztrans_isEnabled($_COOKIE['ztrans_admin_language'])) {
			$q_config['language'] = $_COOKIE['ztrans_admin_language'];
		} else {
	...
	$q_config['language'] = apply_filters('ztranslate_language', $q_config['language']);
	
	// detect language and forward if needed
	if($q_config['detect_browser_language'] && $q_config['url_info']['redirect'] && !isset($_COOKIE['ztrans_cookie_test']) && $q_config['url_info']['language'] == $q_config['default_language']) {
		$target = false;
		...
		if(isset($_SERVER["HTTP_ACCEPT_LANGUAGE"]) && preg_match_all("#([^;,]+)(;[^,0-9]*([0-9\.]+)[^,]*)?#i",$_SERVER["HTTP_ACCEPT_LANGUAGE"], $matches, PREG_SET_ORDER)) {
			...
			foreach($prefered_languages as $language => $priority) {
				...
				if(ztrans_isEnabled($language)) {
					if($q_config['hide_default_language'] && $language == $q_config['default_language']) break;
					$target = ztrans_convertURL(get_option('home'),$language);
					break;
				}
			}
		}
		$target = apply_filters("ztranslate_language_detect_redirect", $target);
		if($target !== false) {
			wp_redirect($target);
			exit();
		}
	...
	if(strpos($q_config['url_info']['url'],'wp-login.php')!==false && defined('AUTH_KEY') && isset($wp_default_secret_key) && $wp_default_secret_key != AUTH_KEY) {
		global $error;
		$error = __('Your $wp_default_secret_key is mismatchting with your AUTH_KEY. This might cause you not to be able to login anymore.', 'ztranslate');
	}
	*/
	...
	if(!defined('WP_ADMIN')) {
		...
		foreach($alloptions as $option => $value) {
			add_filter('option_'.$option, 'ztrans_useCurrentLanguageIfNotFoundUseDefaultLanguage',0);
		}
	}
	
	// load plugin translations
	load_plugin_textdomain('ztranslate', false, dirname(plugin_basename( __FILE__ )).'/lang');
	...
	$q_config['url_info']['url'] = ztrans_convertURL(add_query_arg('lang',$q_config['default_language'],$q_config['url_info']['url']));
}

// returns cleaned string and language information
function ztrans_extractURL($url, $host = '', $referer = '') {
	global $q_config;
	$home = ztrans_parseURL(get_option('home'));
	$home['path'] = trailingslashit($home['path']);
	$referer = ztrans_parseURL($referer);
	...
	{
		setcookie('language', $result['language'], time()+1209600, COOKIEPATH);
	}
	else
	{
		if (isset($_COOKIE['language']))
		{
			$result['language']=$_COOKIE['language'];
		}
		else
		{
	...
	if($referer['host']==$result['host'] && ztrans_startsWith($referer['path'], $home['path'])) {
...
function ztrans_validateBool($var, $default) {
...
function ztrans_loadConfig() {
	...
	$detect_browser_language = ztrans_validateBool($detect_browser_language, $q_config['detect_browser_language']);
	$hide_untranslated = ztrans_validateBool($hide_untranslated, $q_config['hide_untranslated']);
	$auto_update_mo = ztrans_validateBool($auto_update_mo, $q_config['auto_update_mo']);
	$hide_default_language = ztrans_validateBool($hide_default_language, $q_config['hide_default_language']);
	...
	if($permalink_structure===""||strpos($permalink_structure,'?')!==false||strpos($permalink_structure,'index.php')!==false) $url_mode = ZT_URL_QUERY;
	...
	do_action('ztranslate_loadConfig');
}

// saves entire configuration
function ztrans_saveConfig() {
	global $q_config;
	...
	do_action('ztranslate_saveConfig');
}

function ztrans_updateGettextDatabases($force = false, $only_for_language = '') {
	global $q_config;
	...
	foreach($q_config['locale'] as $lang => $locale) {
		if(ztrans_isEnabled($only_for_language) && $lang != $only_for_language) continue;
		if(!ztrans_isEnabled($lang)) continue;
...
function ztrans_updateTermLibrary() {
	global $q_config;
	...
	switch($_POST['action']) {
		...
		case 'add-tag':
		case 'add-link-cat':
			if(isset($_POST['ztrans_term_'.$q_config['default_language']]) && $_POST['ztrans_term_'.$q_config['default_language']]!='') {
				$default = htmlspecialchars(ztrans_stripSlashesIfNecessary($_POST['ztrans_term_'.$q_config['default_language']]), ENT_NOQUOTES);
				if(!isset($q_config['term_name'][$default]) || !is_array($q_config['term_name'][$default])) $q_config['term_name'][$default] = array();
				foreach($q_config['enabled_languages'] as $lang) {
					$_POST['ztrans_term_'.$lang] = ztrans_stripSlashesIfNecessary($_POST['ztrans_term_'.$lang]);
					if($_POST['ztrans_term_'.$lang]!='') {
						$q_config['term_name'][$default][$lang] = htmlspecialchars($_POST['ztrans_term_'.$lang], ENT_NOQUOTES);
					} else {
...
function ztrans_strftime($format, $date, $default = '', $before = '', $after = '') {
...
function ztrans_dateModifiedFromPostForCurrentLanguage($old_date, $format ='') {
	global $post;
	return ztrans_strftime(ztrans_convertDateFormat($format), mysql2date('U',$post->post_modified), $old_date);
}

function ztrans_timeFromPostForCurrentLanguage($old_date, $format = '', $post = null, $gmt = false) {
	$post = get_post($post);
	
	$post_date = $gmt? $post->post_date_gmt : $post->post_date;
	return ztrans_strftime(ztrans_convertTimeFormat($format), mysql2date('U',$post_date), $old_date);
}

function ztrans_timeModifiedFromPostForCurrentLanguage($old_date, $format = '', $gmt = false) {
	global $post;
	$post_date = $gmt? $post->post_modified_gmt : $post->post_modified;
	return ztrans_strftime(ztrans_convertTimeFormat($format), mysql2date('U',$post_date), $old_date);
}

function ztrans_dateFromCommentForCurrentLanguage($old_date, $format ='') {
	global $comment;
	return ztrans_strftime(ztrans_convertDateFormat($format), mysql2date('U',$comment->comment_date), $old_date);
}

function ztrans_timeFromCommentForCurrentLanguage($old_date, $format = '', $gmt = false, $translate = true) {
	if(!$translate) return $old_date;
	global $comment;
	$comment_date = $gmt? $comment->comment_date_gmt : $comment->comment_date;
	return ztrans_strftime(ztrans_convertTimeFormat($format), mysql2date('U',$comment_date), $old_date);
}

/* END DATE TIME FUNCTIONS */

function ztrans_useTermLib($obj) {
	global $q_config;
	if(is_array($obj)) {
		// handle arrays recursively
		foreach($obj as $key => $t) {
			$obj[$key] = ztrans_useTermLib($obj[$key]);
		}
		return $obj;
	}
	if(is_object($obj)) {
...
function ztrans_useAdminTermLib($obj) {
	...
	{
		return ztrans_useTermLib($obj);
	}
}

function ztrans_convertBlogInfoURL($url, $what) {
...
function ztrans_convertURL($url='', $lang='', $forceadmin = false, $showDefaultLanguage = false) {
	...
	if(!ztrans_isEnabled($lang)) return "";
	...
	$urlinfo = ztrans_parseURL($url);
	$home = rtrim(get_option('home'),"/");
	if($urlinfo['host']!='') {
		// check for already existing pre-domain language information
		if($q_config['url_mode'] == ZT_URL_DOMAIN && preg_match("#^([a-z]{2}).#i",$urlinfo['host'],$match)) {
			if(ztrans_isEnabled($match[1])) {
	...
	if(preg_match("#(&|\?)lang=([^&\#]+)#i",$url,$match) && ztrans_isEnabled($match[2])) {
...
function ztrans_split($text, $quicktags = true) {
	global $q_config;
...
function ztrans_join($texts) {
	global $q_config;
	if(!is_array($texts)) $texts = ztrans_split($texts, false);
...
function ztrans_disableLanguage($lang) {
	global $q_config;
	if(ztrans_isEnabled($lang)) {
		$new_enabled = array();
...
function ztrans_enableLanguage($lang) {
	global $q_config;
	if(ztrans_isEnabled($lang) || !isset($q_config['language_name'][$lang])) {
		return false;
	}
	$q_config['enabled_languages'][] = $lang;
	// force update of .mo files
	if ($q_config['auto_update_mo']) ztrans_updateGettextDatabases(true, $lang);
	return true;
}

function ztrans_use($lang, $text, $show_available=false) {
	global $q_config;
	// return full string if language is not enabled
	if(!ztrans_isEnabled($lang)) return $text;
	if(is_array($text)) {
		// handle arrays recursively
		foreach($text as $key => $t) {
			$text[$key] = ztrans_use($lang,$text[$key],$show_available);
		}
		return $text;
	}
	...
	if(!$show_available){
		...
		if($lang!=$q_config['default_language'])
			return "(".$q_config['language_name'][$q_config['default_language']].") ".ztrans_use($q_config['default_language'], $text, $show_available);
		foreach($content as $language => $lang_text) {
			$lang_text = trim($lang_text);
			if(!empty($lang_text)) {
	...
	if(preg_match('/%LANG:([^:]*):([^%]*)%/',$q_config['not_available'][$lang],$match)) {
		...
		foreach($available_languages as $language) {
			...
			$language_list = "<a href=\"".ztrans_convertURL('', $language)."\">".$q_config['language_name'][$language]."</a>".$language_list;
			$i++;
		}
	}
...
function add_ztrans_js ()
{
  wp_register_script( 'ztranslate-script', plugins_url( '/ztranslate.js', __FILE__ ) );
	wp_enqueue_script( 'ztranslate-script' );

}
function add_ztrans_css ()
{
        wp_register_style( 'ztranslate-style', plugins_url('ztranslate.css', __FILE__) );
        wp_enqueue_style( 'ztranslate-style' );
}

function add_ztrans_lang_icons ()
{
	global $q_config;
	echo "<style>\n";
...
function add_ztrans_config ()
{
	global $q_config;
	echo "<script>var zTranslateConfig=".json_encode($q_config).";</script>";
}

$home=get_home_url().'/';
function ztrans_home_url($href)
{
	global $home;
	if ($href===$home)
	{
		return ztrans_convertURL($home);
	}
	else
	{
		return $href;
